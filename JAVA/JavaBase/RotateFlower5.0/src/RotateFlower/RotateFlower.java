...
public class RotateFlower extends Applet{
	...
	private static final int STAGEWIDH = 
			Toolkit.getDefaultToolkit().getScreenSize().width;
	private static final int STAGEHEIGHT = 
	...
	public void paint(Graphics g) {
		...
		if(!isPlaying)
		{
			play(getDocumentBase(),"赖伟锋 - 闹够了没有.wav");
			isPlaying = true;
		}
	}
	...
	public class FlowerOne implements Runnable
	{
		...
		private final float[] pausePosition = {
				89.85f,119.71f,
				177.74f,179.92f,
				239.44f}; 
		private final String[] pauseInfo = {
				"JAVA真神奇！无聊先生，QQ：1096648786",
				"憧憬未来,呵护梦想！",
				"求知若饥，虚心若愚！",
				"为梦想加油！~",
				"残酷,我以青春作为赌注，赢得属于我的梦想."
				};
		private final Point[] pauseInfoPos = {
				new Point(400,400),
				new Point(300,300),
				new Point(200,900),
				new Point(500,300),
		...
		public FlowerOne(
			...
			*******/
			...
			foreGradient = new GradientPaint(
					0,
					0,
					new Color(rC, gC, bC),
					width/2,
					height/2,
					new Color(addConvertColor(rC,+0.5f),
					addConvertColor(gC,-0.5f), 
					addConvertColor(bC,+0.5f)),
					true
					);
		}
		
		private float addConvertColor(float arg,float num)
		{
			if(num>=0)
			{
				num = (float)((((int)(arg*1000) + (int)(num*1000))%1000)/1000.0f);
			}
			else
			{
				num = Math.abs((1000 - ((int)(Math.abs(arg + num)*1000)%1000))/1000.0f);
				//num =Math.abs(255 - Math.abs(arg + num))%255;
			}
			return num;
		}
		...
		public void draw(Graphics g)
		{
			...
			g2D.drawString(TITLE, 180-(TITLE.length()*30/2)+3, 190-10+5);//+3,+5阴影偏移
			g2D.setColor(c);
			...
			Ellipse2D ellipseOne = 
					new Ellipse2D.Double(this.baseX+80,this.baseY+80,w,h);
			Ellipse2D ellipseTwo = 
					new Ellipse2D.Double(this.baseX+80,this.baseY+80,h,w);
			...
			if(isPause)
			{
				drawString(
						...
						pauseInfoPos[pauseCount].x,
						pauseInfoPos[pauseCount].y
						);
			}
		}
		@Override
		public void run() {
			...
			while(true)
			{
				...
				if(cNum == 10)
				{
					rC = addConvertColor(rC,+0.007f);
					gC = addConvertColor(gC,+0.002f);
					bC = addConvertColor(bC,+0.000f);
