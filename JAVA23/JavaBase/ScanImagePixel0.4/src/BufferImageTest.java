...
//		pos.add(temp);
	...
	//	gT = biT.getGraphics();
		for(int i=0; i<pos.size(); i++)
		{
			...
			gT.fillRect(pos.get(i)[0], pos.get(i)[1], 10, 10);
		}
		paint(g);
	}
	...
	public int getProcess(int i){
		return (int)((pos.get(i)[1]+height*pos.get(i)[0])/(.01f*height*width));
	}
	...
	public void run() {
		while(isRun){
			for(int i=0; i<pos.size(); i++){
				...
				String title = "е§дк"+getProcess(i)+"%";
				this.setTitle(title);
...
//					}
//				}
				if(pos.get(i)[1]<height-2){
					pos.get(i)[1]+=2;
				}
				else if(pos.get(i)[0]<width-2){
					pos.get(i)[0]+=2;
					if(pos.get(i)[0]<width-2){
						pos.get(i)[1] = miny;
					}
				}
				else
